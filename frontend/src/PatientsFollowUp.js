          ...
          <tbody>
            {businesses.map((business, index) => (
              <tr key={index} onClick={() => onBusinessClick(business)}>
                ...
                <td>{formatDate(String(business.appointment_date))}</td> {/* Make sure to use appointment_date here */}
                <td>Visit Info</td>
              </tr>
            ))}
          </tbody>
...
function AdminFollow() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/patients?${queryString}`);
